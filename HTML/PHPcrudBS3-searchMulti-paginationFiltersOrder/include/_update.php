...
if(isset($_REQUEST['editId']) and $_REQUEST['editId']!="" and $_REQUEST['table']!=""){
	// $condition to do better
	$row=$db->getAllRecords($_REQUEST['table'],'*',' AND id="'.$_REQUEST['editId'].'"');
...
if(isset($_REQUEST['submit']) and $_REQUEST['submit']!=""){
	...
	extract($_REQUEST);
	if($nom==""){
		header('location:'.$location.'?msg=un&editId='.$_REQUEST['editId'].'&page=0');
		exit;
	}
	elseif($email==""){
		header('location:'.$location.'?msg=ue&editId='.$_REQUEST['editId'].'&page=0');
		exit;
	}
	// elseif($userphone==""){
	// 	header('location:'.$location.'?msg=up&editId='.$_REQUEST['editId'].'&page=0');
	// 	exit;
	// }
	$data	=	array(
					'nom'=>$nom,
	...
	$update	=	$db->update($_REQUEST['table'],$data,array('id'=>$editId));
	if($update){
	...
	<div class="container">
		...
		<div class="panel panel-default">
			<div class="panel-heading clearfix"><i class="fa fa-fw fa-edit"></i> <strong>Edit User</strong> 
				<a href="javascript: history.go(-1)" class="pull-right btn btn-danger btn-sm">
				...
				<div class="col-sm-6">
					...
					<form method="post">
						...
						<div class="form-group">
							...
							<input type="email" name="email" id="email" class="form-control" value="<?php echo $row[0]['email']; ?>" placeholder="Enter user email" required>
						</div>
						<!-- <div class="form-group">
