...
class UploadHandler
{
    ...
    public function __construct($options = null, $initialize = true, $error_messages = null) {
        $this->options = array(
            ...
            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')).'/files/'.$_SESSION["g"].'_'.$_SESSION["signin"].'/',
            // 'upload_url' => $this->get_full_url().'/files/',
            'upload_url' => $this->get_full_url().'/files/'.$_SESSION["g"].'_'.$_SESSION["signin"].'/',
            ...
            'image_versions' => array(
                ...
                */
                'thumbnail' => array(
                    ...
                    'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')).'/files/'.$_SESSION["g"].'_'.$_SESSION["signin"].'/thumbnail/',
                    ...
                    'max_width' => 480, // either specify width, or set to 0. Then width is automatically adjusted - keeping aspect ratio to a specified max_height.
                    'max_height' => 360 // either specify height, or set to 0. Then height is automatically adjusted - keeping aspect ratio to a specified max_width.
                )
            ),
            'print_response' => true
        );
    ...
    protected function upcount_lowercase($matches) {
    ...
    protected function extension_lowercase($name) {
        return preg_replace_callback(
            '/(?:(?: \(([\d]+)\))?(\.[^.]+))?$/',
            array($this, 'upcount_lowercase'),
            $name,
            1
        );
    }
    ...
    protected function fix_file_extension($file_path, $name, $size, $type, $error,
        $index, $content_range) {
            ...
            $name = $this->extension_lowercase($name);
    ...
    protected function trim_file_name($file_path, $name, $size, $type, $error,
        ...
        $name = str_replace('\'', '', $name);
        $name = str_replace('"', '', $name);
        // $name = $this->enleveaccents($name); BUG
        ...
        switch ($type) {
            ...
            case 'png':
                ...
                $image_quality = isset($options['png_quality']) ?
                    $options['png_quality'] : 9; // don't touch why ???
                break;
            default:
                return false;
        }
